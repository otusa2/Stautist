...
-- HELPER: Determine Run Tier based on Roster Levels
...
-- HELPER: Check if whole group is dead
function Stautist:IsGroupWiped()
    ...
    if num == 0 then return UnitIsDeadOrGhost("player") end -- Solo
...
function Stautist:GetGroupSize()
    ...
    if numParty > 0 then return numParty + 1 end -- Party + Player
...
function Stautist:SetupEngine()
    ...
    if self.BossDB then
        ...
        for id, data in pairs(self.BossDB) do
            if data.name then 
                self.NAME_TO_ID[data.name] = id 
                count = count + 1
            end
        end
        print("[Stautist DEBUG] Name Map Built: " .. count .. " zones indexed.")
    end
...
function Stautist:OnLFGComplete()
    -- Failsafe: If LFG says we are done, we are done.
    local state = self.db.char.active_run_state
    if state.is_running then
        self:Print("Dungeon Finder Completion Detected.")
        -- We wait a moment to ensure boss kill logic finished
...
function Stautist:CheckCombatDrop()
    ...
    if state.combat_boss_id then
        ...
        if self:IsGroupWiped() or (deaths > 0 and deaths >= (groupSize - 1)) then
            ...
            if self.UpdateHUDWipes then self:UpdateHUDWipes(state.wipes) end -- New HUD function
        end
        
        -- NEW: If we dropped combat but boss is NOT dead (Reset/Evade)
        -- We assume boss is alive because OnBossKill clears state.combat_boss_id
        if state.combat_boss_id then 
             -- Reset the fight timer so next pull starts fresh
             state.current_fight_start = nil
             state.combat_boss_id = nil
             state.combat_deaths = 0
             -- print("[Debug] Boss Reset. Timer cleared.")
        end
    end
end
...
function Stautist:StopRun(isComplete, reason)
    ...
    local runData = {
        ...
        size = maxPlayers, -- NEW: Save 5, 10, 25, etc.
    ...
    if isComplete then
        ...
        -- Check against other successful runs of same zone/difficulty
        for _, oldRun in ipairs(history) do
            if oldRun ~= runData and oldRun.success and oldRun.zone_id == zID and oldRun.difficulty == runData.difficulty then
                if oldRun.total_time < myTime then
                    rank = rank + 1
                end
            end
        end
        ...
        -- Update HUD with Summary
        if self.ShowRunSummary then
            self:ShowRunSummary(self:FormatTime(finalTime), rank, isPB)
        end
    ...
    state.hud_forced = nil -- Reset "Ask" flag
end
...
function Stautist:OnTimerUpdate(elapsed)
    ...
    if self.hudFrame and self.hudFrame.percentText then
        ...
        if best and best > 0 then
            ...
            self.hudFrame.percentText:SetText("")
        end
    end
    
    -- 3. LIVE SPLITS (NEW)
    -- Only update if order is valid and we have a PB to compare against
...
function Stautist:FormatTime(seconds, precision)
    ...
    local ms = math.floor((seconds - s) * 100) -- 2 digit precision
    ...
    if precision then
        return string.format("%s.%02d", timeStr, ms)
    else
        return timeStr
    end
end
...
function Stautist:CHAT_MSG_SYSTEM(event, msg)
    ...
    -- 1. DETECT RESET (For Instance Limit Logic)
    -- Matches standard "Mana-Tombs has been reset."
    if string.find(msg, "has been reset") then
        self.db.char.reset_detected = true
        -- self:Print("[Debug] Reset Detected. Next entry will count as new.")
    end

    -- 2. EXISTING RESET LOGIC (Stop Run & Announce)
...
function Stautist:OnEnterInstance()
    ...
    -- 2. Instance Limit Tracking (5 per hour) - PER CHARACTER
    local zoneName, instanceType = GetInstanceInfo()
    
    if instanceType == "party" or instanceType == "raid" then
        local now = time()
        
        -- Logic: Is this a new ID?
        local isReEntry = false
        local lastZone = self.db.char.last_instance_name
        local resetHappened = self.db.char.reset_detected
        
        -- If zone name is same as last time AND we haven't reset, assume re-entry
        if lastZone and zoneName == lastZone and not resetHappened then
            isReEntry = true
        end
        
        -- Additional Time Check (Debuff)
        local logDB = self.db.char.instance_log or {}
        local lastEntry = logDB[#logDB]
        if lastEntry and (now - lastEntry.time) < 60 and lastEntry.name == zoneName then
            isReEntry = true
        end
        ...
        if not isReEntry then
            ...
            local entryData = {
                time = now, 
                name = zoneName or GetZoneText() or "Unknown Zone",
                type = instanceType
            }
            ...
            self.db.char.reset_detected = false -- Consumed the reset flag
            ...
            if count >= 4 then
                ...
                local msg = ""
                if count == 4 then
                    msg = string.format("<Stautist> Warning: 4/5 instances entered! Next slot opens in %s.", timeStr)
                elseif count >= 5 then
                    msg = string.format("<Stautist> CRITICAL: 5/5 instances entered (LOCKED). Next slot opens in %s.", timeStr)
                end
...
function Stautist:CountRecentInstances()
    if not self.db.global.instance_log then return 0 end
    local count = 0
    local oneHourAgo = time() - 3600
    for _, log in ipairs(self.db.global.instance_log) do
        if log.time > oneHourAgo then
            count = count + 1
        end
    end
    return count
...
function Stautist:CheckZoneStatus()
    ...
    -- FAILSAFE: If GetInstanceInfo returns nil/empty but we are in a loading transition, abort check
    if not zoneName or zoneName == "" then return end

    -- =========================================================
    -- 1. RELOAD / LAG PROTECTION
    -- =========================================================
    if state.is_running then
        -- If we are running, but info says "none" (LFG lag), check RealZoneText
        if (not instanceType) or (instanceType == "none") then
            ...
            if state.zone_name and currentZone and string.find(currentZone, state.zone_name, 1, true) then
                instanceType = "party" -- Assume we are still inside
                zoneName = state.zone_name
            end
        end
    end
    ...
    if instanceType ~= "party" and instanceType ~= "raid" then
        if state.is_running then
            -- We are marked as running, but API says we are outside.
            -- TRIGGER POPUP (Backup for OnZoneChange)
            local currentZone = GetRealZoneText()
            
            -- Only trigger if RealZoneText confirms we are NOT in the dungeon anymore
            if state.zone_name and currentZone and not string.find(currentZone, state.zone_name, 1, true) then
                 -- FORCE HUD TO STAY VISIBLE
                 self:ShowHUD()
    ...
    if state.is_running and sameZone and difficultyName and difficultyName ~= "" then
        if state.difficulty ~= diffString then
            self:HandleExternalReset("Inside Switch")
        end
    end

    -- =========================================================
    -- 4. START NEW RUN LOGIC
    -- =========================================================
    -- CRITICAL: Only start new run if ID changed. If ID matches, we simply resume (do nothing).
    if (not state.is_running or (dbID and state.zone_id ~= dbID)) and dbID then
        ...
        if self.db.global.run_history then
            ...
            if bestRun then
                ...
                self:Print("PB Loaded for Splits: " .. self:FormatTime(bestTimeFound))
            end
        end
        ...
        if self.db.global.run_history then
            for _, run in pairs(self.db.global.run_history) do
                if run.zone_id == dbID and run.success then totalRuns = totalRuns + 1 end
            end
        end

        self:Print("Run Started: " .. zoneName .. " (" .. self:GetOrdinal(totalRuns + 1) .. " Run)")
    end

    -- =========================================================
    -- 5. HUD REBUILD / RESUME
    -- =========================================================
    if state.is_running and (dbID or state.zone_id) then
        ...
        -- Only rebuild boss list if needed
        local needBuild = true
        if self.hudFrame.bossLines and next(self.hudFrame.bossLines) then
             local checkID = nil
             if self.BossDB[activeID] then 
                 for k,v in pairs(self.BossDB[activeID].bosses) do checkID = k; break end 
             end
        ...
        if needBuild and self.BossDB[activeID] then
            ...
            -- RE-APPLY KILLS (RESUME LOGIC)
            if state.boss_kills then
                for killID, _ in pairs(state.boss_kills) do
...
function Stautist:HandleExternalReset(source)
    ...
    if self.db.profile.announce_resets ~= false then
        ...
        elseif GetNumPartyMembers() > 0 then
            ...
            self:Print("Instance Reset Detected ("..source..").")
        end
    end
...
function Stautist:GetRosterSnapshot()
    ...
    if numRaid == 0 and numParty == 0 then
        ...
        local _, class = UnitClass("player") -- Returns "Warrior", "WARRIOR"
        if name and class then
            table.insert(roster, { name = name, class = class, level = UnitLevel("player") })
        end
        return roster
    end
    ...
    for i = 1, count do
        ...
        -- Only add if name exists (connected)
        if name then
            local _, class = UnitClass(unit)
            table.insert(roster, {
                name = name,
                class = class or "UNKNOWN", -- Safety default
                level = UnitLevel(unit) or 0
            })
        end
    end
    
    -- If party, "partyN" doesn't include player, so add player manually
...
function Stautist:OnBossKill(npcID, bossName, difficulty)
    ...
    if not state.splits_invalid and state.pb_run_data and state.pb_run_data.boss_kills then
        ...
        -- FAILSAFE: Handle String vs Number keys
        local pbBossData = pbKills[npcID] or pbKills[tostring(npcID)]
        
        -- A. Check if this boss exists in PB
        if pbBossData then
            -- B. Check ORDER (Must match sequence)
            -- Sort Current Run by Time (When they died)
            local currentOrder = {}
            for id, data in pairs(state.boss_kills) do 
                if data.time then table.insert(currentOrder, {id=tonumber(id), val=data.time}) end
            end
            table.insert(currentOrder, {id=npcID, val=now}) 
            table.sort(currentOrder, function(a,b) return a.val < b.val end)
            
            -- Sort PB Run by Split Time (When they died relative to start)
            local pbOrder = {}
            for id, data in pairs(pbKills) do 
                -- Failsafe: Use split_time for sort, default to 0 if missing
                local sortVal = data.split_time or 0 
                table.insert(pbOrder, {id=tonumber(id), val=sortVal}) 
            end
            ...
            if currentIndex <= #pbOrder then
                ...
                -- Allow ID mismatch ONLY if we are skipping bosses (handled by splits_invalid later)
                -- But for strict splits:
                if pbBoss.id ~= curBoss.id then
                    -- Detect Out of Order
                    self:Print("|cffff0000[Stautist]|r Kill Order Deviation. Splits invalid.")
                    state.splits_invalid = true
                    if self.ClearSplits then self:ClearSplits() end
                else
                    -- MATCH! Calculate Final Split
                    if pbBossData.split_time then
                        splitDiff = runSplit - pbBossData.split_time
                    end
                end
            else
                state.splits_invalid = true -- More kills than PB has?
            end
        end
    end
    ...
    if state.is_running then
        ...
        -- Update HUD Split Text (Final Static Value)
    ...
    if bossID then
        ...
        if type(bData) == "table" then
            ...
            elseif bData.hidden and self.currentZoneID == 608 then
                 -- VIOLET HOLD LOGIC
                 -- 1. Check if this boss is already assigned (re-engage)
                 local assignedSlot = nil
                 
                 -- We check existing kills to see if slots are taken
                 local slot1_taken = false
                 for kID, _ in pairs(state.boss_kills) do
                     -- If we have kills, we assume they took slots in order. 
                     -- But this is Engage event. We might not have killed yet.
                     -- We need a runtime "engaged slot" tracker or check HUD text.
                 end
                 
                 -- Simplified: Check HUD Text
                 if self.hudFrame and self.hudFrame.bossLines then
                     ...
                     elseif row1 and row1.name:GetText() == bData.name then
                         -- Already assigned to 1, do nothing
        ...
        -- Start Timer
        if not state.current_fight_start then
            local isHostile = (subEvent:find("_DAMAGE") or subEvent:find("_CAST_") or subEvent == "RANGE_DAMAGE" or subEvent == "SWING_DAMAGE")
            if isHostile then
...
function Stautist:CheckFastestKill(npcID, duration, name, difficulty)
    ...
    if self.db.global.run_history then
        for _, run in ipairs(self.db.global.run_history) do
            -- Match Zone/Diff (Optional: Do we compare Normal vs Heroic kills? Usually yes, separate them)
    ...
    -- If this is a new record (and we have history, or it's the first run ever)
    if duration < bestDuration then
        local diff = bestDuration - duration
        local msg = ""
        if not hasHistory or bestDuration == 99999 then
             -- First time recording duration
             -- msg = string.format("Recorded kill for %s: %s", name, self:FormatTime(duration))
        else
             msg = string.format("NEW RECORD! %s killed in %s (-%s)", name, self:FormatTime(duration), self:FormatTime(diff))
